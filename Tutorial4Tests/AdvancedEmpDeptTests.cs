...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnMaxSalary()
    {
        ...
        decimal? maxSalary = emps.Select(e => e.Sal).Max(); 
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        ...
        var result = emps.Join(emps, 
            e1 => e1.Mgr, e2 => e2.EmpNo, (e1, e2) => new { e1, e2 })
            .Select(e1e2 => new {Employee = e1e2.e1.EName, Manager = e1e2.e2.EName});
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(depts,
            ...
            .Join(grades, ed => 1, s => 1, (ed, s) => new { ed, s })
            .Where(eds => eds.ed.e.Sal >= eds.s.Losal && eds.ed.e.Sal <= eds.s.Hisal)
            .Select(eds => new { EName = eds.ed.e.EName, DName = eds.ed.d.DName, Grade = eds.s.Grade });
